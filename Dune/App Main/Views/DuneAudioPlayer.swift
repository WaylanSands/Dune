...
//    }
    ...
    func downloadEpisodeAudio(audioID: String, completion: @escaping (URL) -> ()) {
        ...
        playPauseButton.setImage(UIImage.init(named: "download-arrow-black"), for: .normal)
        playbackButton.imageEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
    ...
    func updateYPositionWith(value: CGFloat) {
        yPosition = value
        if !isOutOfPosition {
            self.frame.origin.y = value - playerHeight
        }
    }
