...
class EpisodeFeedCell: UITableViewCell {
    ...
    lazy var epCaptionTextView: UITextView = {
        let view = UITextView()
    ...
    func normalSetUp(episode: Episode) {
        ...
        epCaptionTextView.text = episode.caption
        programImageButton.setImage(episode.image, for: .normal)
...
//        setupAccountLabel()
        ...
        DispatchQueue.main.async {
            ...
            self.addGradient()
            if self.epCaptionTextView.lineCount() > 3 {
                self.addMoreButton()
            }
        }
    }
    ...
    func refreshSetupMoreTapFalse() {
        epCaptionTextView.textContainer.maximumNumberOfLines = 3
        moreButton.isHidden = false
        moreButtonPress = false
    }
    
    func refreshSetupMoreTapTrue() {
        epCaptionTextView.textContainer.maximumNumberOfLines = 0
        epCaptionTextView.textContainer.exclusionPaths.removeAll()
    ...
    func setupViews() {
        ...
        topRightTopStackView.addArrangedSubview(programNameLabel)
...
//        accountLabel.backgroundColor = .blue
        ...
        topRightStackView.addArrangedSubview(captionView)
        captionView.translatesAutoresizingMaskIntoConstraints = false
        captionView.leadingAnchor.constraint(equalTo: topRightStackView.leadingAnchor).isActive = true
        captionView.trailingAnchor.constraint(equalTo: topRightStackView.trailingAnchor).isActive = true

        captionView.addSubview(epCaptionTextView)
        epCaptionTextView.translatesAutoresizingMaskIntoConstraints = false
        epCaptionTextView.topAnchor.constraint(equalTo: captionView.topAnchor).isActive = true
        epCaptionTextView.leadingAnchor.constraint(equalTo: captionView.leadingAnchor).isActive = true
        epCaptionTextView.trailingAnchor.constraint(equalTo: captionView.trailingAnchor, constant: -10).isActive = true
//        summaryTextView.backgroundColor = .blue


        self.addSubview(tagScrollView)
        tagScrollView.translatesAutoresizingMaskIntoConstraints = false
        tagScrollView.topAnchor.constraint(equalTo: epCaptionTextView.bottomAnchor, constant: 5).isActive = true
        tagScrollView.leadingAnchor.constraint(equalTo: epCaptionTextView.leadingAnchor).isActive = true
        tagScrollView.trailingAnchor.constraint(equalTo: epCaptionTextView.trailingAnchor, constant: -20).isActive = true
...
//        tagScrollView.backgroundColor = .red
        ...
        gradientOverlayView.trailingAnchor.constraint(equalTo: epCaptionTextView.trailingAnchor, constant: -20).isActive = true
...
//        releaseTime.heightAnchor.constraint(equalToConstant: 20).isActive = true
    ...
    func addMoreButton() {
        ...
        moreButton.bottomAnchor.constraint(equalTo: epCaptionTextView.bottomAnchor).isActive = true
        moreButton.trailingAnchor.constraint(equalTo: epCaptionTextView.trailingAnchor).isActive = true
        moreButton.heightAnchor.constraint(equalToConstant: epCaptionTextView.font!.lineHeight).isActive = true
        self.bringSubviewToFront(moreButton)

        let rect = CGRect(x: epCaptionTextView.frame.width - moreButton.intrinsicContentSize.width - 5, y: epCaptionTextView.frame.height - epCaptionTextView.font!.lineHeight, width: moreButton.intrinsicContentSize.width, height: epCaptionTextView.font!.lineHeight - 2)
        let path = UIBezierPath(rect: rect)
        epCaptionTextView.textContainer.exclusionPaths = [path]
    }
...
//        programNameLabel.attributedText = programNameAttributedString
//    }
    ...
    @objc func moreUnwrap() {
        ...
        DispatchQueue.main.async {
            self.epCaptionTextView.updateConstraints()
