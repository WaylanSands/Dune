...
class addChannelNameVC: UIViewController {
    ...
    @objc func continueButtonPress() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let channelID = NSUUID().uuidString
            ...
            let channelRef = db.collection(publisherType.channel.rawValue).document(User.channelID!)
            
            userRef.updateData([
                "channelID": User.channelID!
            ]) { (error) in
                if let error = error {
                    print("There has been an error adding channel ID: \(error.localizedDescription)")
                } else {
                    ...
                    channelRef.setData([
                        ...
                        if let error = error {
                            print("There has been an error adding the channel: \(error.localizedDescription)")
                        } else {
    ...
    func presentNextVC() {
        if let imageController = UIStoryboard(name: "OnboardingPublisher", bundle: nil).instantiateViewController(withIdentifier: "ImageController") as? publisherImageVC {
            imageController.accountType = publisherType.channel.rawValue
...
extension addChannelNameVC: UITextFieldDelegate {
